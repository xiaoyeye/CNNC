...
save_dir = os.path.join(os.getcwd(),'data_no_label')
...
for i in range(len(gene_pair_index)-1):   #### many sperations
    ...
    for gene_pair in gene_pair_label_array[start_index:end_index]: ## each speration
        ...
        x_gene_name, y_gene_name = separation[0], separation[1]#x_gene_name,y_gene_name,label = separation[0],separation[1],separation[2]

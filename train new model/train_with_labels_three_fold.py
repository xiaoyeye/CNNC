...
# Usage  python train_with_labels_three_fold.py
# command line in developer's linux machine :
# module load cuda-8.0 using GPU
#srun -p gpu --gres=gpu:1 -c 2 --mem=20Gb python train_with_labels_three_fold.py 3057 > results.txt
#######################OUTPUT
...
for test_indel in range(1,4): ################## three fold cross validation
    ...
    (x_test, y_test,count_set) = load_data_TF2(test_TF)
    print(x_train.shape, 'x_train samples')
    print(x_test.shape, 'x_test samples')
    save_dir = os.path.join(os.getcwd(),str(test_indel)+'_saved_models_T_32-32-64-64-128-128-512_e'+str(epochs))
